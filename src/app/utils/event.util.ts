 ...
 * @param type type
 */
export function isKey(event: KeyboardEvent, type: AvailableKey): boolean | void {
  if (event) {
...
/**
 * return the scrolling direction with `deltaY`
 * @param deltaY `deltaY` of `WheelEvent`
 */
export function getWheelDirection(deltaY: number): WheelDirection {
  if (deltaY > 0) {
 ...
 */
export function getDistanceOfFingers(first: Touch | null, second: Touch | null): number {
  let distance = 0;
