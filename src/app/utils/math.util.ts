 ...
 * @param y point y
 */
export function isCircleContainsPoint(cx: number, cy: number, r: number, x: number, y: number): boolean {
...
/**
 * get the point list of line
 * the line will be divided by length
 * from https://community.esri.com/t5/arcgis-api-for-javascript/point-along-a-line/td-p/479200
 * @param start start of line
 * @param end end of line
 * @param length length of slope to divide the line
 */
export function getPointsOfLine(start: Point, end: Point, length: number): Point[] {
  const angle = getAngleOfLine(start, end);
  const c = getLineLength(start, end);
  const points: Point[] = [];

  // i is slope length
  for (let i = 0; i <= c; i += length) {
    points.push({
 ...
 * the arc will be divided by length
 * @param center center point of circle
 * @param start start angle of arc
 * @param end end angle of arc
 * @param radius radius of circle
 * @param length arc length to divide
 */
export function getPointsOfArc(
  center: Point,
  start: number,
  end: number,
  radius: number,
...
/**
 * get x,y point on arc with specific angle
